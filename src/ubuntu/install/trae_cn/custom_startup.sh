#!/usr/bin/env bash
set -ex
START_COMMAND="trae-cn"

TRAE_WORKDIR=${TRAE_WORKDIR:-"/trae-cn"}
mkdir -p ${TRAE_WORKDIR}/trae
[[ -e ~/.trae ]] || ln -s ${TRAE_WORKDIR}/trae ~/.trae
mkdir -p ${TRAE_WORKDIR}/trae-cn
[[ -e ~/.trae-cn ]] || ln -s ${TRAE_WORKDIR}/trae-cn ~/.trae-cn
mkdir -p ${TRAE_WORKDIR}/trae-cn-server
[[ -e ~/.trae-cn-server ]] || ln -s ${TRAE_WORKDIR}/trae-cn-server ~/.trae-cn-server
...
PGREP_PATTERN="trae-cn"
export MAXIMIZE="true"
export MAXIMIZE_NAME="Trae CN"
